...
class multihandler:
	...
	def list_sessions(self):
		if not self.sessions_id:
			h.info_general("No Sessions Established!")
		else:
	...
	def show_commands(self):
		commands = [
			("Interact","Interacts With The Active Session."),
			("Terminate","Terminates The Active Session."),
			("Sessions","List All Available Sessions."),
			("Exit","Terminate All Sessions & Go Back To Main Menu."),
		]
		...
		while 1:
			try:
				...
				if cmd == "Interact":
					...
					h.info_error("Invalid Response --> " + cmd )
